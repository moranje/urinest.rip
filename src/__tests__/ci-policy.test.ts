...
const frameworkPackagePlan = read("docs/framework-package-plan-2026-06-01.md");
...
describe("CI policy", () => {
  ...
  it("keeps docs aligned with current package and theme architecture", () => {
    ...
    expect(frameworkPackagePlan).toContain("@moranje/beslismodel@0.1.1");
    expect(frameworkPackagePlan).toContain("CI draait als single Node 24 lane");
    expect(frameworkPackagePlan).toContain("GitHub CI gebruikt GitHub Packages auth");
    expect(frameworkPackagePlan).toContain("Ronde 12");
    expect(frameworkPackagePlan).not.toContain("App consumeert `@moranje/beslismodel@0.1.0`");
    expect(frameworkPackagePlan).not.toContain("CI matrix.");
    expect(frameworkPackagePlan).not.toContain("GitHub Actions configureert de Gitea npm registry");
